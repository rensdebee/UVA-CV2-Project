...
class StableDiffusion(nn.Module):
    def __init__(self, device, vram_O, fp16=True, t_range=[0.02, 0.98], max_t_range=0.98, num_train_timesteps=None, 
        ...
        print(f'[INFO] loading ISM stable diffusion...')
    ...
    def get_text_embeds(self, prompts, negative_prompts):
        ...
        inverse_emebd = self.encode_text(self.opt.inverse_text)
        self.embeddings['pos'] = pos_embeds
        self.embeddings['neg'] = neg_embeds
        self.embeddings['default'] = pos_embeds
        self.embeddings['uncond'] = neg_embeds
        self.embeddings['inverse_text'] = inverse_emebd
        self.text_z_inverse = torch.cat([self.embeddings['uncond'], self.embeddings['inverse_text']], dim=0)
    ...
    def train_step(self, pred_rgb, step_ratio, pred_depth=None, pred_alpha=None,
                    grad_scale=1,use_control_net=False,
                    save_folder:Path=None, warm_up_rate = 0,
                    resolution=(512, 512), guidance_opt=None,as_latent=False):
        
        iteration = step_ratio
        batch_size = pred_rgb.shape[0]
        pred_rgb = pred_rgb.to(self.dtype)
        pred_rgb = F.interpolate(pred_rgb, (512, 512), mode='bilinear', align_corners=False)
        pred_rgb = self.augmentation(pred_rgb)[0]
        text_embeddings = torch.cat([self.embeddings['pos'].expand(batch_size, -1, -1), self.embeddings['neg'].expand(batch_size, -1, -1)])
        B = pred_rgb.shape[0]
        ...
        embedding_inverse = self.text_z_inverse
        ...
        warm_up_rate = 1. - min(iteration/self.opt.warmup_iter,1.)
        current_delta_t =  int(self.opt.delta_t + (warm_up_rate)*(self.opt.delta_t_start - self.opt.delta_t))
        ...
        with torch.no_grad():
            ...
            if not self.ism:
                ...
                xs_delta_t = self.opt.xs_delta_t if self.opt.xs_delta_t is not None else current_delta_t
                xs_inv_steps = self.opt.xs_inv_steps if self.opt.xs_inv_steps is not None else int(np.ceil(ind_prev_t / xs_delta_t))
                ...
                _, prev_latents_noisy, pred_scores_xs = self.add_noise_with_cfg(latents, noise, ind_prev_t, starting_ind, inverse_text_embeddings, 
                                                                                self.opt.denoise_guidance_scale, xs_delta_t, xs_inv_steps, eta=self.opt.xs_eta)
                # Step 2: sample x_t
                _, latents_noisy, pred_scores_xt = self.add_noise_with_cfg(prev_latents_noisy, noise, ind_t, ind_prev_t, inverse_text_embeddings, 
                                                                           self.opt.denoise_guidance_scale, current_delta_t, 1, is_noisy_latent=True)        
        ...
        pred_noise = noise_pred_uncond + self.opt.guidance_scale * delta_DSD
