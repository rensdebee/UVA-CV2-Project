...
class StableDiffusion(nn.Module):
    ...
    def train_step(
        ...
        with torch.no_grad():
            ...
            current_delta_t =  self.guidance_opt.delta_t
            ...
            text_z_inverse =torch.cat([self.embeddings['uncond'],self.embeddings['inverse_text']], dim=0)
            inverse_text_embeddings = text_z_inverse.unsqueeze(1).repeat(1, B, 1, 1).reshape(-1, text_z_inverse.shape[-2], text_z_inverse.shape[-1])
