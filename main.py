...
class GUI:
    ...
    def prepare_train(self):
        ...
        if self.guidance_sd is None and self.enable_sd:
            if self.opt.mvdream:
                if self.opt.stage1 == "ISM":
                    ...
                    print(f"[INFO] loaded MVDream! (ISM)")
                if self.opt.stage1 == "VSD":
                    print(f"[INFO] loading MVDream (VSD)...")
                    from guidance.mvdream_VSD_utils import MVDream

                    self.guidance_sd = MVDream(self.device, guidance_opt=self.opt)
                    print(f"[INFO] loaded MVDream! (VSD)")
                else:
                    print(f"[INFO] loading MVDream (SDS)...")
                    from guidance.mvdream_utils import MVDream

                    self.guidance_sd = MVDream(self.device)
                    print(f"[INFO] loaded MVDream! (SDS)")
                ...
                self.guidance_sd = StableDiffusion(
                    self.device, None, None, guidance_opt=self.opt
                )
        ...
        with torch.no_grad():
            ...
            if self.opt.stage1 == "VSD":
                params = [
                    {"params": self.guidance_sd.lora_layers.parameters()},
                    {"params": self.guidance_sd._unet.camera_emb.parameters()},
                    {"params": self.guidance_sd._unet.lambertian_emb},
                    {"params": self.guidance_sd._unet.textureless_emb},
                    {"params": self.guidance_sd._unet.normal_emb},
                ]
                self.unet_optimizer = torch.optim.AdamW(params, lr=0.0001)  # naive adam
                warm_up_lr_unet = lambda iter: (
                    iter / (500 * 1 + 1) if iter <= (500 * 1 + 1) else 1
                )
                self.unet_scheduler = torch.optim.lr_scheduler.LambdaLR(
                    self.unet_optimizer, warm_up_lr_unet
                )
    ...
    def train_step(self):
        ...
        for _ in range(self.train_steps):
            ...
            if self.opt.mvdream:
                ...
                elif self.opt.stage1 == "VSD":
                    loss_gd, latents, poses = self.guidance_sd.train_step(
                        images,
                        poses,
                        step_ratio=step_ratio if self.opt.anneal_timestep else None,
                    )
                    loss = loss + self.opt.lambda_sd * loss_gd
                    if self.step % 1 == 0:
                        for _ in range(1):
                            self.unet_optimizer.zero_grad()
                            timesteps = torch.randint(
                                0, 1000, (1,), device=self.device
                            ).long()  # temperarily hard-coded for simplicity
                            with torch.no_grad():
                                latents_clean = latents.contiguous()
                                pose = poses
                                if random.random() < 0.1:
                                    pose = torch.zeros_like(poses)
                            noise = torch.randn(latents_clean.shape, device=self.device)
                            latents_noisy = self.guidance_sd.scheduler.add_noise(
                                latents_clean, noise, timesteps
                            )
                            model_output = self.guidance_sd.unet(
                                latents_noisy, timesteps, c=pose, shading="albedo"
                            ).sample
                            loss_unet = F.mse_loss(
                                model_output,
                                self.guidance_sd.scheduler.get_velocity(
                                    latents_clean, noise, timesteps
                                ),
                            )
    ...
    def save_model(self, mode="geo", texture_size=1024):
        ...
        if mode == "geo":
            ...
            wandb.log({"generated_samples": wandb.Object3D(open(path))})
