...
def coarse_training_with_density_regularization(args):
    ...
    # Opacity management
    prune_low_opacity_gaussians_at = [7500]
    if bind_to_surface_mesh:
    ...
    for batch in range(9_999_999):
        ...
        for i in range(0, train_num_images, train_num_images_per_batch):
            ...
            if not no_rendering:
                ...
                # torchvision.transforms.v2.ToPILImage()(img1).show()
                ...
                if regularize:
                    ...
                    if iteration > regularize_from:
                        ...
                        if regularize_sdf and iteration > start_sdf_regularization_from:
                            ...
                            if (
                                    use_sdf_estimation_loss or enforce_samples_to_be_on_surface) and iteration > start_sdf_estimation_from:
                                ...
                                if use_projection_as_estimation:
                                    ...
                                    if backpropagate_gradients_through_depth:
                                        ...
                                        with torch.no_grad():
                                            ...
                                            depth = sugar.render_image_gaussian_rasterizer(
                                                camera_indices=camera_indices.item(),
                                                bg_color=max_depth + torch.zeros(3, dtype=torch.float,
                                                                                 device=sugar.device),
                                                sh_deg=0,
                                                compute_color_in_rasterizer=False,
                                                # compute_color_in_rasterizer,
                                                compute_covariance_in_rasterizer=True,
                                                return_2d_radii=False,
                                                use_same_scale_in_all_directions=False,
                                                point_colors=point_depth,
                                ...
                                if sample_only_in_gaussians_close_to_surface:
                                    with torch.no_grad():
                                        gaussian_to_camera = torch.nn.functional.normalize(
                                            fov_camera.get_camera_center() - sugar.points, dim=-1)
                                        ...
                                        gaussian_centers_map_z = sugar.get_points_depth_in_depth_map(fov_camera, depth,
                                                                                                     gaussian_centers_in_camera_space)

                                        gaussian_standard_deviations = (
                                                sugar.scaling * quaternion_apply(quaternion_invert(sugar.quaternions),
                                                                                 gaussian_to_camera)
                                        ).norm(dim=-1)
                                ...
                                if (use_sdf_estimation_loss or enforce_samples_to_be_on_surface) and iteration > start_sdf_estimation_from:
                                    ...
                                    if use_projection_as_estimation:
                                        ...
                                        samples_gaussian_normals = sugar.get_normals(estimate_from_points=False)[
                                            sdf_gaussian_idx]
                                        sdf_estimation = ((sdf_samples - sugar.points[
            ...
            if iteration == 1 or iteration % print_loss_every_n_iterations == 0:
                ...
                with torch.no_grad():
                    ...
                    if regularize_sdf and iteration > start_sdf_regularization_from:
                        CONSOLE.print("Number of gaussians used for sampling in SDF regularization:",
            ...
            if do_sh_warmup and (iteration > 0) and (current_sh_levels < sh_levels) and (
                    iteration % sh_warmup_every == 0):
                current_sh_levels += 1
                CONSOLE.print("Increasing number of spherical harmonics levels to", current_sh_levels)

            if do_resolution_warmup and (iteration > 0) and (current_resolution_factor > 1) and (
                    iteration % resolution_warmup_every == 0):
